...
anova(length_site)
emm<- emmeans(length_site, specs = "Site")
pairs(emm, specs="Site", adjust="none")
emmeans(length_site, specs = "Site", adjust = "none")
multcomp::cld(emm) 
site_emm <- emmeans(length_site, pairwise ~ Site, adjust = "tukey")
site_emm
library(emmeans)
anova_model <- aov(Length_mm ~ Site, data = data)
anova_model
em <- emmeans(anova_model, ~ Site)
cld_results <- cld(em, Letters = letters, adjust = "none")
